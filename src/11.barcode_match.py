...
win_name = 'Camera Matching' # 윈두우 이름 
MIN_MATCH = 10 # 최소 매칭점 개수( 이값 이하면 매칭 실패로 간주)

# ORB 검출기 생성
# ORB_cretea(1000)는 이미지에서 1000개의 특징점을 찾는 알고리즘 
...
while cap.isOpened():       
    ...
    if img1 is None:  # 등록된 이미지 없음, 카메라 바이패스
        ...
        if desc1 is None or desc2 is None or len(desc1) < 2 or len(desc2) < 2:
            ...
            # [step 3]
            # k=2로 knnMatch : 각 특징점만다 가장 유사한 2개의 후보를 찾음 
            ...
            for match_pair in matches:
                if len(match_pair) == 2:
                    ...
                    if m.distance < n.distance * ratio: # 1등이 2등보다 25%이상좋으면 
            ...
            if len(good_matches) > MIN_MATCH: 
                ...
                #src_pts = np.float32([kp1[m.queryIdx].pt for m in good_matches]).reshape(0, 1, 2)
